...
-- =============================================================================
-- netrw workaround: keep the <leader> popup working inside netrw.
-- ------------------------------------------------------------------------------
-- which-key v3 has a bug with netrw: when netrw opens (or re-opens its reused
-- buffer) the native auto-trigger for <leader> often stops firing, so pressing
-- <leader> shows nothing or the wrong menu.
--
-- Fix: on every netrw FileType event, forcibly install a buffer-local <leader>
-- mapping that explicitly opens which-key at the LEADER level (not the root
-- level). We use `keys = " "` because vim.g.mapleader is a literal space.
-- A pcall guards against any remaining edge-cases and falls back to the root
-- menu if necessary.
-- =============================================================================
vim.api.nvim_create_autocmd("FileType", {
  ...
  callback = function(ev)
    local buf = ev.buf
    vim.keymap.set("n", "<leader>", function()
      -- Try to jump straight to the <leader> submenu.
      local ok = pcall(wk.show, { keys = " ", loop = false })
      if not ok then
        -- Ultimate fallback: open the root menu so the user at least sees *something*.
        wk.show()
      end
    end, {
      buffer = buf,
